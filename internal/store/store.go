...
type Store struct {
	...
	nextStream uint64
	pendingLockDeletes map[model.LockID]model.Revision
}
...
func (s *Store) DeleteLockState(id model.LockID) (model.Revision, error) {
	current := s.CurrentRev()
	s.mu.RLock()
	_, ok := s.records[lockKey(id)]
	s.mu.RUnlock()
	if !ok {
		return current, nil
	}
	pending := s.pendingLockDeletes
	if pending == nil {
		pending = make(map[model.LockID]model.Revision)
	}
	pending[id] = current
	s.pendingLockDeletes = pending
